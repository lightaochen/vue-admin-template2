...
class Mail:
    ...
    def add_html_spread(self, html_text: str):
        """
        添加HTML内容。将 <img> 占位符自动替换为 cid 图像嵌入。
        """
        img_count = html_text.count("<img>")
        if img_count > self.img_id:
            raise ValueError(f"HTML中要求插入 {img_count} 张图片，但你只添加了 {self.img_id} 张，请先调用 add_img")

        # 顺序替换 <img> -> <img src="cid:N">
        for i in range(1, img_count + 1):
            html_text = html_text.replace("<img>", f'<img src="cid:{i}">', 1)
    ...
    def add_excel_attachment(self, file_name: str):
        with open(file_name, 'rb') as fn:
            file_data = fn.read()
            ...
            attachment.add_header(
                'Content-Disposition',
                'attachment',
                filename=os.path.basename(file_name)  # 保留真实文件名
            )
